...
var (
	...
	strConnection = "NEWSDBPG_TEST" //используем тестовую БД
	strInform     = "PostgreSQL"
)

// проверяем соединение
func TestNew(t *testing.T) {
	t.Run("New pg: Good (valid connection)", func(t *testing.T) {
	...
	t.Run("New pg: Error (invalid connection)", func(t *testing.T) {
...
func TestStore_NewsAndAddNew(t *testing.T) {
	...
	t.Run("AddNew pg: Good (add one line with error)", func(t *testing.T) {
		err := store.AddNew([]storage.News{testNews1})
		assert.NoError(t, err)
	})

	t.Run("News pg: Good (get records from real bd)", func(t *testing.T) {
